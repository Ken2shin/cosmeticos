    ...
    try {
      await sql`
        ...
        DO UPDATE SET 
          ...
          total_spent = customers.total_spent + ${total_amount},
          last_purchase_date = CURRENT_TIMESTAMP,
          phone = COALESCE(EXCLUDED.phone, customers.phone),
          name = COALESCE(EXCLUDED.name, customers.name)
      `
      console.log("[v0] Orders API: Customer record updated/created")
    } catch (customerError) {
      console.error("[v0] Orders API: Error updating customer record:", customerError)
    }
