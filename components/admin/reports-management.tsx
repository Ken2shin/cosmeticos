...
export function ReportsManagement() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/profits?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`)
      ...
      let message = "Error desconocido. Verifica que existan pedidos en el período seleccionado."
      if (error instanceof Error) {
        if (error.message.includes('relation "profit_tracking" does not exist')) {
          message = "La tabla de seguimiento de ganancias no existe en la base de datos. Por favor, crea la tabla o verifica la configuración del backend."
        } else {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/reports/profits/pdf?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`)
