...
function Button({
  ...
  const hasOnlyIcon =
    ...
    React.Children.toArray(children).every((child) => React.isValidElement(child) && typeof child.type === "function")

  return (
    <Comp
      data-slot="button"
      className={cn(buttonVariants({ variant, size, className }))}
      aria-label={hasOnlyIcon && !props["aria-label"] ? "Botón de acción" : props["aria-label"]}
      {...props}
    >
      {children}
    </Comp>
