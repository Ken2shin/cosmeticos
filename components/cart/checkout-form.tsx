      ...
      if (response.ok) {
        ...
        await fetch("/api/notifications/broadcast", {
          ...
          body: JSON.stringify({
            ...
            message: `${formData.name} envió una lista de pedido por $${state.total.toFixed(2)}`,
            data: {
              orderId: order.id,
      ...
      <div className="flex flex-col items-center justify-center h-full space-y-6 p-8">
        ...
        <div className="text-center space-y-3 max-w-md">
          ...
          <div className="bg-rose-50 p-6 rounded-lg mt-6 border border-rose-200">
            <p className="text-rose-800 font-bold text-xl">Total: ${state.total.toFixed(2)}</p>
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 mt-1">
                      Cantidad: <span className="font-semibold">{item.quantity}</span> × ${item.price}
                    </div>
                  </div>
